...
def babyshark_main():
    ...
    try:
        ...
        cfg.BABYSHARK_PROCESS_ACTIVE = True
    ...
    if args.version:
        ...
        try:
            while cfg.BABYSHARK_PROCESS_ACTIVE:
                raw_data, addr = __socket__.recvfrom(65536)
                dest_mac, src_mac, eth_proto, data = nf.unpack_ether_frame(raw_data)

                if(packet_source=='192.168.111.139' or packet_destination=='192.168.111.139'):
                    print(BOLD + G + "[babyshark]>" + W + BOLD + 'Ethernet Frame: ')
                    esp.print_babyshark_notab('Destination: {}, Source: {}, Protocol: {}\n'.format(dest_mac, src_mac, eth_proto))
                    try:
                        ...
                        if eth_proto == 8:
                            ...
                            if packet_protocol == 1:
                                ...
                                if args.filename:
                                    ...
                                    if(icmp_packet_type=='69'):
                                        icmp_packet_type = "echo"
                                    PCAP(pcap_file_name).write_to_pcap_file("ICMP Checksum: {}, ICMP Type: {}, ICMP Code: {}\n\n".format(icmp_check_summation, icmp_packet_type,icmp_packet_code))
                                else: pass
                                ...
                                if len(data) > cfg.ESPI_SUCCESS_CODE_STANDARD and args.httpraw:
                                    if segment_source_port == cfg.ESPI_HTTP_DEFAULT_PORT or segment_destination_port == cfg.ESPI_HTTP_DEFAULT_PORT:
                                        ...
                                        esp.print_babyshark_noprefix('\t\t' + "Raw TCP/Raw-no-http Packet Bytes: ")
                                        print(babyshark_textwrapper('\t\t\t', data))
                                else: pass
                                if args.filename:
        ...
        try:
            while True:
                raw_data, addr = __socket__.recvfrom(65536)
                dest_mac, src_mac, eth_proto, data = nf.unpack_ether_frame(raw_data)
                str1 = list(raw_data[66:])
                string = ''.join(map(chr,str1))
                ...
                try:
                    ...
                    if(packet_source=='192.168.111.139' or packet_destination=='192.168.111.139'):
                        if eth_proto == 8:
                            ...
                            if args.filename:
                                    PCAP(pcap_file_name).write_to_pcap_file("\nIPv4 Packet Contents {}".format(cfg.ESPI_ASCII_DOWN_ARROW))
                                    PCAP(pcap_file_name).write_to_pcap_file("\t <-- Protocol: {}, Source: {}, Destination: {} --> ".format(protocol, packet_source, packet_destination))
                            else: pass
                            ...
                            if packet_protocol == 1:
                                ...
                                if args.filename:
                                    ...
                                    PCAP(pcap_file_name).write_to_pcap_file("\t\tICMP Data: {}\n\n".format(icmp_packet_data))
                                else: pass
                                ...
                                if len(data) > cfg.ESPI_SUCCESS_CODE_STANDARD and args.httpraw:
                                    if segment_source_port == cfg.ESPI_HTTP_DEFAULT_PORT or segment_destination_port == cfg.ESPI_HTTP_DEFAULT_PORT:
                                        ...
                                        esp.print_babyshark_noprefix('\t\t' + "Raw TCP/Raw-no-http Packet Bytes: ")
                                        print(babyshark_textwrapper('\t\t\t', raw_data))
                                else: pass
                                ...
                                if args.filename:
                                    ...
                                    PCAP(pcap_file_name).write_to_pcap_file('\n\t\t' + "Raw TCP/Raw-no-http Packet Bytes:\n")
                                    PCAP(pcap_file_name).write_to_pcap_file(babyshark_textwrapper('\t\t\t', raw_data))
                                    PCAP(pcap_file_name).write_to_pcap_file('\n\t\t' + "Raw TCP/Raw-no-http byte  to String :\n\t\t")
                                    PCAP(pcap_file_name).write_to_pcap_file('\n++++\n' +string+'++++')
                                    PCAP(pcap_file_name).write_to_pcap_file('\n\n'+'-'*100+'\n')
        ...
        esp.print_babyshark_message("Visited URLs will be displayed below.\n", True)
        ...
        try:
            ...
            print(BOLD + R + "\n[!] Exiting Babyshark HTTPS Interception.\n" + END)
