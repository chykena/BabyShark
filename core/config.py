...
class Config:
    ...
    BABYSHARK_PROCESS_ACTIVE = False
...
class Babyshark(object):
    def print_babyshark_message(self, msg, color=True):
        babyshark = '\n[babyshark]>'
        icon = '[*] '
        if color:
            print(BOLD + G + babyshark + R + msg + END)
        else: print(icon + msg)

    def print_babyshark_notab(self, msg, color=True):
        babyshark = '[babyshark]>'
        icon = '[*] '
        if color:
    ...
    def print_babyshark_noprefix(self, msg, color=True):
        babyshark = '[babyshark]>'
        if color:
            cprint(msg, 'green', attrs=['bold'])
        else: print('\t' + msg)

    def print_babyshark_malformed_packet(self, msg, color=True):
        if color:
...
class Platform(object):
    ...
    def GetOperatingSystemDescriptor(self):
        ...
        if sys.platform == "win32" or sys.platform == "win64":
            ...
            e.print_babyshark_notab("OS: Windows | (" + p.GetUsernameDescriptor() + '@' +
                p.GetHostnameDescriptor() + ")", color=True)

        if sys.platform == "darwin":
            cfg.ESPI_OSYSTEM_DARWIN_OSX = True
            e.print_babyshark_notab("OS: OSX/Darwin | (" + p.GetUsernameDescriptor() + '@' +
                p.GetHostnameDescriptor() + ")", color=True)

        if sys.platform == "linux" or sys.platform == "linux2":
            cfg.ESPI_OSYSTEM_UNIX_LINUX = True
            e.print_babyshark_notab("OS: Unix/Linux | (" + p.GetUsernameDescriptor() + '@' +
                p.GetHostnameDescriptor() + ")", color=True)

    def babysharkClear(self):
        time.sleep(0.7)
        if sys.platform == "win32" or sys.platform == "win64":
            os.system("cls")
...
def babyshark_textwrapper(prefix, string, size=80):
        size -= len(prefix)
