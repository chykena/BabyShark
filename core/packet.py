...
class Packet(object):
    ...
    def handle_ipv4_packet(self, packet_data):
        ...
        try:
            ...
            esp = Babyshark()
            esp.print_babyshark_malformed_packet("\t {} MALFORMED PACKET >> Passing...\n".format(cfg.ESPI_ASCII_DOWN_RIGHT_ARROW))
            pass
    ...
    def handle_raw_http_packet(self, packet_data):
        ...
        esp = Babyshark()
        esp.print_babyshark_noprefix("\t\tRaw HTTP Packet: ")
        try:
            ...
            for order in byteorder_information:
                esp.print_babyshark_noprefix('\t\t\t' + str(order))
        except:
            print(babyshark_textwrapper('\t\t\t', packet_data))
...
def process_http_packet(httppacket):
    ...
    try:
        if httppacket.haslayer(HTTPRequest):
            ...
            esp.print_babyshark_noprefix(f"[+] {packet_ip_address} <requested> {packet_url} with {http_method}", color=True)
            if httppacket.haslayer(Raw) and http_method == "POST":
                pretty_raw_data = str(httppacket[Raw]).strip("{}b")
                esp.print_babyshark_notab(f"{cfg.ESPI_ASCII_DOWN_ARROW} > Raw Data (possible credentials): " + pretty_raw_data.replace('&', ' | '))
    except IndexError:
        pass
